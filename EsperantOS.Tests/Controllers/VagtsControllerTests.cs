...
public class VagtsControllerTests
{
    ...
    /// <summary>Bygger en UoW-mock forudkonfigureret med fælles standardværdier.</summary>
        ...
        Medarbejder? medarbejder   = null)
    {
        ...
        // EnsureAedruVagterAsync har altid brug for GetFridayVagterAsync
    ...
    public async Task Index_ReturnsViewWithSortedVagter()
    {
        ...
        Assert.True(model[0].Dato <= model[1].Dato); // sorteret stigende
    }
    ...
    {
        var uow        = MakeUoW(); // intet opsat for id 99
        var controller = BuildController(uow);
    ...
    public async Task Create_Post_NonFriday_DoesNotCreate()
    {
        ...
        // Bør redirecte tilbage til Create (ikke Index) pga. modelfejl
        var redirect = Assert.IsType<RedirectToActionResult>(result);
        Assert.Equal("Create", redirect.ActionName);
        uow.Object.VagtRepository.GetType(); // sikr at Add aldrig blev kaldt
    }
    ...
    public async Task TagVagt_KnownUser_AssignsUserToVagtAndReturnsOk()
    {
        ...
        // Første kald af TagVagt, andet kald inde i UpdateVagtAsync
    ...
    public async Task TagVagt_UnknownUser_AutoCreatesEmployeeAndAssigns()
    {
        ...
        // Første opslag returnerer null; andet (efter oprettelse) returnerer den nye bruger
        mockMed.SetupSequence(r => r.GetMedarbejderByNameAsync("NyBruger"))
               .ReturnsAsync((Medarbejder?)null)
               .ReturnsAsync(newUser);

        // UpdateVagtAsync henter hver medarbejder via id fra MedarbejderRepository
    ...
    {
        var uow    = MakeUoW(); // id 99 ikke opsat
